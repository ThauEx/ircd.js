...
class Channel {
  ...
  onInviteList(user) {
    let userNick = this.server.normalizeName(user.nick);

    return this.inviteList.some(nick => {
      return this.server.normalizeName(nick) === userNick;
    });
  }
  ...
  send() {
    let message = arguments.length === 1 ? arguments[0] : Array.prototype.slice.call(arguments).join(' ');
  ...
  addModes(user, modes, arg) {
    ...
    modes.slice(1).split('').forEach(mode => {
...
Channel.prototype = {
  addMode: {
    o: function(user, arg) {
      if (user.isOp(this)) {
        let targetUser = this.findUserNamed(arg);
